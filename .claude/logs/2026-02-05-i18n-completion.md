...
## 2026-02-05 追加修正（第2回コミット）

### 問題

前回コミット（80fb19d）で app.js の変更がコミットされていなかったことが判明。
ユーザーから「プロフィールモーダルやボタンが日本語に戻っている」と報告。

### 原因

commit 80fb19d では locale ファイルと game.html のみがコミットされ、
app.js と mypage.js の変更がステージングされていなかった。

### 修正内容

**app.js:**
- リミックス coming soon アラート
- 共有テキスト
- ドキュメントタイトル（ゲームクリエイター）
- バージョン復元メッセージ
- 処理中パーセント表示
- ゲーム完成/エラー通知
- 変更箇所を見るボタン
...
- `4954d7c` fix(i18n): complete app.js and mypage.js i18n support
- デプロイ完了（ヘルスチェック OK）
